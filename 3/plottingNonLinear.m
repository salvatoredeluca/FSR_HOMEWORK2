...
grid on
hold on
title('Bernoulli lemniscate trajectory', 'Interpreter', 'latex')
...
% --- Sezione commentata lasciata intatta ---
% figure(2)
...
legend('Tracked path', 'Reference', 'Initial position', 'Interpreter', 'latex', 'Location', 'best');
xlabel('x[m]', 'Interpreter', 'latex')
ylabel('y[m]', 'Interpreter', 'latex')
xlim([xd_min - 0.8, xd_max + 0.8]);
ylim([yd_min - 0.8, yd_max + 0.8]);
...
figure(7)
plot(out.e1.Time(:), sqrt(out.e1.Data(:).^2 + out.e2.Data(:).^2 + out.e3.Data(:).^2), ...
